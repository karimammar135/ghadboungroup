import*as i0 from"@angular/core";import{EventEmitter,Component,ChangeDetectionStrategy,ViewEncapsulation,Input,Output,ViewChild,HostBinding,NgModule}from"@angular/core";import*as i1 from"@angular/common";import{CommonModule}from"@angular/common";const createElement=data=>{if(data){var{attributes:data,element:_element,elementName,value}=data,element=_element||document.createElement(elementName||"div");if(data&&element instanceof HTMLElement)for(var[n,v]of Object.entries(data))element.setAttribute(n,String(v));if(value)if("string"==typeof value)element.innerHTML=value;else for(const child of Array.isArray(value)?value:[value]){var actualChild=createElement(child);actualChild&&element.appendChild(actualChild)}return element}},rPx=/px$/,cssWithoutPx=css=>Number(css.replace(rPx,"")),buildErrorMessage=message=>"twicpics-components "+message,mappingBoolean$1={true:!0,false:!1,"":!0},convertToBoolean=value=>"boolean"==typeof value?value:void 0!==value&&(mappingBoolean$1[value.trim()]||void 0),debounce=(fn,options)=>{let timer;const _options=Object.assign({leading:!0,ms:0,trailing:!0},options);return(...args)=>{!timer&&_options.leading&&fn(...args),clearTimeout(timer),timer=setTimeout(()=>{timer=void 0,_options.trailing&&fn(...args)},_options.ms)}},isReact=!1,isReactNative=!1,isWebComponents=!1,isBrowser=isWebComponents||"undefined"!=typeof document,logError=message=>{console.error(buildErrorMessage(message))},logWarning=message=>{console.warn(buildErrorMessage(message))},noop=()=>{},regExpFinderFactory=(regExp,{filter,defaultValue}={})=>value=>{let found;return value&&(""+value).replace(regExp,(_,v)=>found=v),(filter?filter(found):found)||defaultValue},throwError=message=>{throw new Error(buildErrorMessage(message))},trimRegExpFactory=(items,{border="\\s",regExpFlags}={})=>new RegExp(`^(?:${border})*(${Array.isArray(items)?items.join("|"):items})(?:${border})*$`,regExpFlags),defaultConfig={breakpoints:{xs:320,sm:640,md:768,lg:1024,xl:1280,"2xl":1536},class:"twic",domain:void 0,env:"production",handleShadowDom:noop,maxDPR:void 0,path:"",step:void 0},w=isBrowser&&window,config=isBrowser?w["~ TPCC"]||(w["~ TPCC"]=defaultConfig):defaultConfig,configBasedStyle=()=>`.twic-w>.${config.class}-background-done+div,.twic-w>.${config.class}-done+div,.twic-w>.${config.class}-poster-done+div{opacity:0 !important}.twic-w>.${config.class}-done,.twic-w>.${config.class}-poster-done{transform:none !important}`,getDataAttributeName=baseName=>`data-${config.class}-`+baseName,handleShadowDomFactory=attributeName=>{const marked=new WeakSet;return item=>{for(;item&&!marked.has(item);){marked.add(item);var parentNode=item["parentNode"];if(!parentNode&&item instanceof ShadowRoot){if("closed"===item.mode)throw new Error("cannot use TwicPics components in closed ShadowRoot");createElement({element:item,value:{elementName:"style",value:"*STYLE*"+configBasedStyle()}}),(item=item.host)&&item.setAttribute(attributeName,"")}else item=parentNode}}},DEFAULT_MAX_DPR=2,setConfig=options=>{var{breakpoints:options={},debug,domain,class:_class,env,handleShadowDom,maxDPR,path}=options;config.breakpoints=Object.assign(Object.assign({},config.breakpoints),options),config.class=_class||config.class,config.domain=domain,config.env=debug?"debug":env,config.maxDPR=Math.max(1,maxDPR||DEFAULT_MAX_DPR),config.path=path,config.handleShadowDom=handleShadowDom&&isBrowser&&!isReactNative?handleShadowDomFactory(getDataAttributeName("component")):noop},rPath=/^(?:(auth|placeholder|rel)|(image|media|video)|[^:]*):(\/*)((v[0-9]+(?=[/?]))?[^?]*(\?.*)?)$/,FULL_PATH=4,MEDIA=2,QUERY=6,RESERVED=5,SPECIAL=1,VERSION$1="v1",urlInfos=(src,domain="")=>{var parsed=src&&rPath.exec(src);return{isAbsolute:src.slice(0,domain.length+1)===domain+"/",isSpecial:parsed&&void 0!==parsed[SPECIAL]}},computeTransform=({height,mode,width},transform)=>{return transform&&(width||height)?transform.replace(/(\/*\*)/g,"/"+mode+`=${mode=width||"-"}x`+(width=height||"-")).replace(/WxH/g,mode+"x"+width):transform},createUrl=({domain,context,inspect,output,quality,src,transform})=>{var isAbsolute=urlInfos(src,domain)["isAbsolute"],isAbsolute=isAbsolute?"media:"+src.slice((domain+"/").length):src,src=rPath.exec(isAbsolute),isMedia=src&&src[MEDIA],output=output?"/output="+output:"",isAbsolute=isMedia?src[FULL_PATH]:isAbsolute,quality=quality?"/quality="+quality:"",inspect=inspect?"/inspect":"",context=computeTransform(context,transform);return domain+"/"+(src&&(src[SPECIAL]||src[RESERVED])?""+VERSION$1+context+output+quality+inspect+"/"+(isMedia?src[MEDIA]+":"+isAbsolute:isAbsolute):`${isAbsolute}${src&&src[QUERY]?"&":"?"}twic=`+VERSION$1+context+output+inspect+quality)},finalTransform=(mode,refit)=>"cover"===mode&&void 0!==refit?"":"/*",validAnchors=["center"];for(const y of["","bottom","top"])for(const x of["","left","right"])(x||y)&&validAnchors.push(y?x?y+"-"+x:y:x);const rInvalidPath=/\?|^\/*$/,rValidAnchor=trimRegExpFactory(validAnchors),rValidDomain=/(^https?:\/\/[^/]+)\/*$/,validFetchPriorities=["high","low","auto"],rValidFetchPriority=trimRegExpFactory(validFetchPriorities),rValidIntrinsic=trimRegExpFactory("\\s*(\\d+)\\s*[x]\\s*(\\d+)\\s*"),validModes=["contain","cover"],rValidMode=trimRegExpFactory(validModes),validPlaceholders=["maincolor","meancolor","none","preview"],rValidPlaceholder=trimRegExpFactory(validPlaceholders),rValidPath=/^\/*(.+?)\/*$/,rValidRatio=trimRegExpFactory("(\\d+(?:\\.\\d+)?)(?:\\s*[\\/:]\\s*(\\d+(?:\\.\\d+)?))?|(none)"),validEnvironment=["debug","offline","production"],rValidEnvironment=trimRegExpFactory(validEnvironment),rValidZoom=trimRegExpFactory("(\\d+\\.?\\d*)|(css)",{regExpFlags:"i"}),isPositiveNumber=value=>!isNaN(value)&&0<value,rMedia=/^((image|media|video):)?\/*/,trimOrUndefined=regExpFinderFactory(trimRegExpFactory(`(?:.|
    ...
    `,styleUrls:["./_/style.css"],changeDetection:ChangeDetectionStrategy.OnPush,encapsulation:ViewEncapsulation.None,host:{ngSkipHydration:"true"}}]}],ctorParameters:function(){return[{type:i0.Renderer2}]},propDecorators:{alt:[{type:Input}],anchor:[{type:Input}],bot:[{type:Input}],className:[{type:Input}],focus:[{type:Input}],intrinsic:[{type:Input}],mode:[{type:Input}],mediaTag:[{type:Input}],eager:[{type:Input}],placeholder:[{type:Input}],position:[{type:Input}],preTransform:[{type:Input}],ratio:[{type:Input}],refit:[{type:Input}],src:[{type:Input}],step:[{type:Input}],title:[{type:Input}],transition:[{type:Input}],transitionDelay:[{type:Input}],transitionDuration:[{type:Input}],transitionTimingFunction:[{type:Input}],videoOptions:[{type:Input}],stateChangeEvent:[{type:Output}],containerRef:[{type:ViewChild,args:["container",{static:!0}]}],placeholderRef:[{type:ViewChild,args:["placeholderElement",{static:!1}]}]}});class TwicBackgroundComponent{constructor(){this.anchor=void 0,this.bot=void 0,this.focus=void 0,this.intrinsic=void 0,this.mediaTag="div",this.mode=void 0,this.placeholder=void 0,this.position=void 0,this.preTransform=void 0,this.ratio=void 0,this.step=void 0,this.transitionDelay=void 0,this.transitionDuration=void 0,this.transitionTimingFunction=void 0,this.stateChangeEvent=new EventEmitter}onStateChange(stateEvent){this.stateChangeEvent.emit(stateEvent)}}TwicBackgroundComponent.ɵfac=i0.ɵɵngDeclareFactory({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicBackgroundComponent,deps:[],target:i0.ɵɵFactoryTarget.Component}),TwicBackgroundComponent.ɵcmp=i0.ɵɵngDeclareComponent({minVersion:"12.0.0",version:"12.2.0",type:TwicBackgroundComponent,selector:"TwicBackground",inputs:{anchor:"anchor",bot:"bot",focus:"focus",intrinsic:"intrinsic",mediaTag:"mediaTag",mode:"mode",eager:"eager",placeholder:"placeholder",position:"position",preTransform:"preTransform",ratio:"ratio",src:"src",step:"step",transition:"transition",transitionDelay:"transitionDelay",transitionDuration:"transitionDuration",transitionTimingFunction:"transitionTimingFunction"},outputs:{stateChangeEvent:"stateChangeEvent"},host:{classAttribute:"twic-i twic-d"},ngImport:i0,template:`
        <TwicMedia
            [anchor]="anchor"
            [bot]="bot"
            [focus]="focus"
        ...
        ></TwicMedia>
    `,isInline:!0,components:[{type:TwicMediaComponent,selector:"TwicMedia",inputs:["alt","anchor","bot","className","focus","intrinsic","mode","mediaTag","eager","placeholder","position","preTransform","ratio","refit","src","step","title","transition","transitionDelay","transitionDuration","transitionTimingFunction","videoOptions"],outputs:["stateChangeEvent"]}],changeDetection:i0.ChangeDetectionStrategy.OnPush,encapsulation:i0.ViewEncapsulation.None}),i0.ɵɵngDeclareClassMetadata({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicBackgroundComponent,decorators:[{type:Component,args:[{selector:"TwicBackground",template:`
        <TwicMedia
            [anchor]="anchor"
            [bot]="bot"
            [focus]="focus"
            ...
            (stateChangeEvent)="onStateChange($event)"
        ></TwicMedia>
    `,changeDetection:ChangeDetectionStrategy.OnPush,encapsulation:ViewEncapsulation.None,host:{class:"twic-i twic-d"}}]}],propDecorators:{anchor:[{type:Input}],bot:[{type:Input}],focus:[{type:Input}],intrinsic:[{type:Input}],mediaTag:[{type:Input}],mode:[{type:Input}],eager:[{type:Input}],placeholder:[{type:Input}],position:[{type:Input}],preTransform:[{type:Input}],ratio:[{type:Input}],src:[{type:Input}],step:[{type:Input}],transition:[{type:Input}],transitionDelay:[{type:Input}],transitionDuration:[{type:Input}],transitionTimingFunction:[{type:Input}],stateChangeEvent:[{type:Output}]}});const ease=(t,d)=>{return t<0||d<t?Math.min(Math.max(0,t),d):(t=2*t/d)<1?d/2*Math.pow(t,3):(t-=2,d/2*(Math.pow(t,3)+2))},move=e=>{e.preventDefault(),e.stopPropagation();var currentTarget=e.currentTarget,{left,top,right,bottom}=currentTarget.getBoundingClientRect(),{clientX:e,clientY}=e instanceof MouseEvent?e:e.touches[0];currentTarget.style.setProperty("--twic-xr",ease(e-left,right-left).toString()),currentTarget.style.setProperty("--twic-yr",ease(clientY-top,bottom-top).toString())};var initMagnifier=magnifierContainer=>{magnifierContainer=magnifierContainer.firstElementChild;magnifierContainer.addEventListener("mousemove",e=>move(e)),magnifierContainer.addEventListener("touchmove",e=>move(e))};class TwicImgComponent{constructor(renderer,hostElement){this.renderer=renderer,this.hostElement=hostElement,this.mediaTag="img",this.alt=void 0,this.anchor=void 0,this.bot=void 0,this.focus=void 0,this.intrinsic=void 0,this.mode=void 0,this.placeholder=void 0,this.position=void 0,this.preTransform=void 0,this.ratio=void 0,this.step=void 0,this.title=void 0,this.transitionDelay=void 0,this.transitionDuration=void 0,this.transitionTimingFunction=void 0,this.zoom=void 0,this.stateChangeEvent=new EventEmitter,this._zoom=!1}get twicZoom(){return this._zoom}ngAfterViewInit(){this._zoom&&initMagnifier(this.hostElement.nativeElement.firstElementChild),this.updateTemplate()}onStateChange(stateEvent){this.stateChangeEvent.emit(stateEvent)}ngOnChanges(){this._zoom=parseZoom(this.zoom),this.magnifierStyle=computeMagnifierStyle(this._zoom),this.updateTemplate()}updateTemplate(){var _a;null!=(_a=this.hostElement)&&_a.nativeElement&&styles(this.magnifierStyle,this.hostElement.nativeElement,this.renderer)}}TwicImgComponent.ɵfac=i0.ɵɵngDeclareFactory({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicImgComponent,deps:[{token:i0.Renderer2},{token:i0.ElementRef}],target:i0.ɵɵFactoryTarget.Component}),TwicImgComponent.ɵcmp=i0.ɵɵngDeclareComponent({minVersion:"12.0.0",version:"12.2.0",type:TwicImgComponent,selector:"TwicImg",inputs:{alt:"alt",anchor:"anchor",bot:"bot",focus:"focus",intrinsic:"intrinsic",mode:"mode",eager:"eager",placeholder:"placeholder",position:"position",preTransform:"preTransform",ratio:"ratio",refit:"refit",src:"src",step:"step",title:"title",transition:"transition",transitionDelay:"transitionDelay",transitionDuration:"transitionDuration",transitionTimingFunction:"transitionTimingFunction",zoom:"zoom"},outputs:{stateChangeEvent:"stateChangeEvent"},host:{properties:{"class.twic-z":"this.twicZoom"},classAttribute:"twic-i twic-d"},usesOnChanges:!0,ngImport:i0,template:`
        <TwicMedia
            *ngIf="_zoom"
            [anchor]="anchor"
            [bot]="bot"
        ...
        <TwicMedia
            ...
            (stateChangeEvent)="onStateChange($event)"
        ></TwicMedia>
    `,isInline:!0,components:[{type:TwicMediaComponent,selector:"TwicMedia",inputs:["alt","anchor","bot","className","focus","intrinsic","mode","mediaTag","eager","placeholder","position","preTransform","ratio","refit","src","step","title","transition","transitionDelay","transitionDuration","transitionTimingFunction","videoOptions"],outputs:["stateChangeEvent"]}],directives:[{type:i1.NgIf,selector:"[ngIf]",inputs:["ngIf","ngIfThen","ngIfElse"]},{type:i1.NgStyle,selector:"[ngStyle]",inputs:["ngStyle"]}],changeDetection:i0.ChangeDetectionStrategy.OnPush,encapsulation:i0.ViewEncapsulation.None}),i0.ɵɵngDeclareClassMetadata({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicImgComponent,decorators:[{type:Component,args:[{selector:"TwicImg",template:`
        <TwicMedia
            *ngIf="_zoom"
            [anchor]="anchor"
            [bot]="bot"
        ...
        <TwicMedia
            ...
            (stateChangeEvent)="onStateChange($event)"
        ></TwicMedia>
    `,changeDetection:ChangeDetectionStrategy.OnPush,encapsulation:ViewEncapsulation.None,host:{class:"twic-i twic-d"}}]}],ctorParameters:function(){return[{type:i0.Renderer2},{type:i0.ElementRef}]},propDecorators:{alt:[{type:Input}],anchor:[{type:Input}],bot:[{type:Input}],focus:[{type:Input}],intrinsic:[{type:Input}],mode:[{type:Input}],eager:[{type:Input}],placeholder:[{type:Input}],position:[{type:Input}],preTransform:[{type:Input}],ratio:[{type:Input}],refit:[{type:Input}],src:[{type:Input}],step:[{type:Input}],title:[{type:Input}],transition:[{type:Input}],transitionDelay:[{type:Input}],transitionDuration:[{type:Input}],transitionTimingFunction:[{type:Input}],zoom:[{type:Input}],stateChangeEvent:[{type:Output}],twicZoom:[{type:HostBinding,args:["class.twic-z"]}]}});class TwicPictureComponent{constructor(renderer){this.renderer=renderer,this.alt=void 0,this.anchor=void 0,this.fetchpriority=void 0,this.focus=void 0,this.mode=void 0,this.position=void 0,this.preTransform=void 0,this.ratio=void 0,this.title=void 0,this._alt=void 0,this._anchors=void 0,this._fetchpriority=void 0,this._focuses=void 0,this._modes=void 0,this._positions=void 0,this._preTransform=void 0,this._ratios=void 0,this._refit=void 0,this._sizes=void 0,this._sources=[],this._title=void 0}ngAfterViewInit(){var _a;null!=(_a=this.pictureData)&&_a.sources&&(this._sources=this.pictureData.sources.map(_=>{var _source=this.renderer.createElement("source");return this.renderer.insertBefore(this.containerRef.nativeElement,_source,this.imageRef.nativeElement),_source})),this.updateMedia()}ngOnChanges(){this._alt=parseAlt(this.alt),this._anchors=parseAnchors(this.anchor),this._focuses=parseFocuses(this.focus),this._modes=parseModes(this.mode),this._eager=parseEager(this.eager),this._positions=parsePositions(this.position),this._preTransform=parsePreTransform(this.preTransform),this._ratios=parseRatios(this.ratio),this._refit=parseRefit(this.refit),this._sizes=parseSizes(this.sizes),this._src=parseSrc(this.src),this._title=parseTitle(this.title),this.description=computeAlt(this._alt,"img"),this.pictureData=Object.assign({},computePicture(this._anchors,this._eager,this._fetchpriority,this._focuses,this._modes,this._positions,this._preTransform,this._ratios,this._refit,this._sizes,this._src)),this.updateMedia()}updateMedia(){var _a,_b;this.imageRef.nativeElement&&attributes(null==(_a=this.pictureData)?void 0:_a.img,this.imageRef.nativeElement,this.renderer);for(const index in this._sources){var actualSourceElement=this._sources[index];attributes(null==(_b=this.pictureData)?void 0:_b.sources[index],actualSourceElement,this.renderer)}}}TwicPictureComponent.ɵfac=i0.ɵɵngDeclareFactory({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicPictureComponent,deps:[{token:i0.Renderer2}],target:i0.ɵɵFactoryTarget.Component}),TwicPictureComponent.ɵcmp=i0.ɵɵngDeclareComponent({minVersion:"12.0.0",version:"12.2.0",type:TwicPictureComponent,selector:"TwicPicture",inputs:{alt:"alt",anchor:"anchor",eager:"eager",fetchpriority:"fetchpriority",focus:"focus",mode:"mode",position:"position",preTransform:"preTransform",ratio:"ratio",refit:"refit",sizes:"sizes",src:"src",title:"title"},host:{attributes:{ngSkipHydration:"true"}},viewQueries:[{propertyName:"containerRef",first:!0,predicate:["container"],descendants:!0,static:!0},{propertyName:"imageRef",first:!0,predicate:["image"],descendants:!0,static:!0}],usesOnChanges:!0,ngImport:i0,template:`
      <picture
          #container
          class="twic-p"
          [attr.title]="_title"
  ...
  `,isInline:!0,styles:['.twic-i{overflow:hidden}.twic-w,.twic-w *{border:none;margin:0;overflow:hidden;padding:0}.twic-w{overflow:hidden;position:relative;width:100%;padding-top:calc(100% / 1);padding-top:calc(100% / var( --twic-ratio, 1 ))}.twic-w>*{display:block;height:100%;left:0;position:absolute;top:0;width:100%;transition-property:opacity,transform;will-change:opacity,transform;-o-object-fit:cover;object-fit:cover;-o-object-fit:var( --twic-mode,cover );object-fit:var( --twic-mode,cover );-o-object-position:center;object-position:center;-o-object-position:var( --twic-position,center );object-position:var( --twic-position,center );transition-delay:0s;transition-delay:var( --twic-transition-delay,0s );transition-duration:.4s;transition-duration:var( --twic-transition-duration,.4s );transition-timing-function:ease;transition-timing-function:var( --twic-transition-timing-function,ease )}.twic-w>*,.twic-w>div{background-size:cover;background-position:50%;background-repeat:no-repeat}.twic-w>div{background-size:var( --twic-mode,cover );background-position:var( --twic-position,center );font-size:calc(1px / 1);font-size:calc(1px / var( --twic-ratio, 1 ))}.twic-w>img:not([src]),.twic-w>img[src=""]{visibility:hidden}.twic-w.twic-tz>img{transform:scale(0)}.twic-w.twic-tf>div{opacity:1}.twic-d{display:block}.twic-offline{background-color:#ccc}.twic-offline.twic-nosrc{background-color:#fd0016}.twic-offline>*{display:none}:root{--twic-zoom:0}.twic-z{position:relative}.twic-m{left:0;position:absolute;top:0;z-index:1}.twic-m>*{display:none;transition:none;width:calc(max(0, 1) * 100%);width:calc(max(var( --twic-zoom ), 1) * 100%);height:calc(max(0, 1) * 100%);height:calc(max(var( --twic-zoom ), 1) * 100%);transform:translate3d(calc((1 - max(0, 1)) * 0 * 1px),calc((1 - max(0, 1)) * 0 * 1px),0);transform:translate3d(calc((1 - max(var( --twic-zoom ), 1)) * var( --twic-xr, 0) * 1px),calc((1 - max(var( --twic-zoom ), 1)) * var( --twic-yr, 0) * 1px),0)}.twic-m:hover>*{display:block}.twic-m:hover+div,twicmedia:hover+twicmedia{opacity:0}.twic-p{border:none;margin:0;overflow:hidden;padding:0}.twic-p>img{display:block;-o-object-fit:cover;object-fit:cover;-o-object-position:center;object-position:center;width:100%;height:100%}'],changeDetection:i0.ChangeDetectionStrategy.OnPush,encapsulation:i0.ViewEncapsulation.None}),i0.ɵɵngDeclareClassMetadata({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicPictureComponent,decorators:[{type:Component,args:[{selector:"TwicPicture",template:`
      <picture
          #container
          class="twic-p"
          [attr.title]="_title"
  ...
  `,styleUrls:["./_/style.css"],changeDetection:ChangeDetectionStrategy.OnPush,encapsulation:ViewEncapsulation.None,host:{ngSkipHydration:"true"}}]}],ctorParameters:function(){return[{type:i0.Renderer2}]},propDecorators:{alt:[{type:Input}],anchor:[{type:Input}],eager:[{type:Input}],fetchpriority:[{type:Input}],focus:[{type:Input}],mode:[{type:Input}],position:[{type:Input}],preTransform:[{type:Input}],ratio:[{type:Input}],refit:[{type:Input}],sizes:[{type:Input}],src:[{type:Input}],title:[{type:Input}],containerRef:[{type:ViewChild,args:["container",{static:!0}]}],imageRef:[{type:ViewChild,args:["image",{static:!0}]}]}});class TwicVideoComponent{constructor(){this.mediaTag="video",this.anchor=void 0,this.bot=void 0,this.duration=void 0,this.focus=void 0,this.from=void 0,this.intrinsic=void 0,this.mode=void 0,this.placeholder=void 0,this.position=void 0,this.posterFrom=void 0,this.preTransform=void 0,this.ratio=void 0,this.step=void 0,this.title=void 0,this.to=void 0,this.transitionDelay=void 0,this.transitionDuration=void 0,this.transitionTimingFunction=void 0,this.stateChangeEvent=new EventEmitter}ngOnChanges(){this._duration=parseDuration(this.duration),this._from=parseFrom(this.from),this._posterFrom=parseDuration(this.posterFrom),this._to=parseTo(this.to),this.videoOption=preComputeVideoOptions(this._duration,this._from,this._posterFrom,this._to)}onStateChange(stateEvent){this.stateChangeEvent.emit(stateEvent)}}TwicVideoComponent.ɵfac=i0.ɵɵngDeclareFactory({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicVideoComponent,deps:[],target:i0.ɵɵFactoryTarget.Component}),TwicVideoComponent.ɵcmp=i0.ɵɵngDeclareComponent({minVersion:"12.0.0",version:"12.2.0",type:TwicVideoComponent,selector:"TwicVideo",inputs:{anchor:"anchor",bot:"bot",duration:"duration",focus:"focus",from:"from",intrinsic:"intrinsic",mode:"mode",eager:"eager",placeholder:"placeholder",position:"position",posterFrom:"posterFrom",preTransform:"preTransform",ratio:"ratio",src:"src",step:"step",title:"title",to:"to",transition:"transition",transitionDelay:"transitionDelay",transitionDuration:"transitionDuration",transitionTimingFunction:"transitionTimingFunction"},outputs:{stateChangeEvent:"stateChangeEvent"},host:{classAttribute:"twic-i twic-d"},usesOnChanges:!0,ngImport:i0,template:`
        <TwicMedia
            [anchor]="anchor"
            [bot]="bot"
            [focus]="focus"
            ...
            (stateChangeEvent)="onStateChange($event)"
        ></TwicMedia>
    `,isInline:!0,components:[{type:TwicMediaComponent,selector:"TwicMedia",inputs:["alt","anchor","bot","className","focus","intrinsic","mode","mediaTag","eager","placeholder","position","preTransform","ratio","refit","src","step","title","transition","transitionDelay","transitionDuration","transitionTimingFunction","videoOptions"],outputs:["stateChangeEvent"]}],changeDetection:i0.ChangeDetectionStrategy.OnPush,encapsulation:i0.ViewEncapsulation.None}),i0.ɵɵngDeclareClassMetadata({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicVideoComponent,decorators:[{type:Component,args:[{selector:"TwicVideo",template:`
        <TwicMedia
            [anchor]="anchor"
            [bot]="bot"
            [focus]="focus"
            ...
            (stateChangeEvent)="onStateChange($event)"
        ></TwicMedia>
    `,changeDetection:ChangeDetectionStrategy.OnPush,encapsulation:ViewEncapsulation.None,host:{class:"twic-i twic-d"}}]}],propDecorators:{anchor:[{type:Input}],bot:[{type:Input}],duration:[{type:Input}],focus:[{type:Input}],from:[{type:Input}],intrinsic:[{type:Input}],mode:[{type:Input}],eager:[{type:Input}],placeholder:[{type:Input}],position:[{type:Input}],posterFrom:[{type:Input}],preTransform:[{type:Input}],ratio:[{type:Input}],src:[{type:Input}],step:[{type:Input}],title:[{type:Input}],to:[{type:Input}],transition:[{type:Input}],transitionDelay:[{type:Input}],transitionDuration:[{type:Input}],transitionTimingFunction:[{type:Input}],stateChangeEvent:[{type:Output}]}});class TwicViewComponent{constructor(renderer,elementRef){this.renderer=renderer,this.elementRef=elementRef}ngOnInit(){this.renderer.setAttribute(this.elementRef.nativeElement,getDataAttributeName("view"),"")}}TwicViewComponent.ɵfac=i0.ɵɵngDeclareFactory({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicViewComponent,deps:[{token:i0.Renderer2},{token:i0.ElementRef}],target:i0.ɵɵFactoryTarget.Component}),TwicViewComponent.ɵcmp=i0.ɵɵngDeclareComponent({minVersion:"12.0.0",version:"12.2.0",type:TwicViewComponent,selector:"TwicView",ngImport:i0,template:"<ng-content></ng-content>",isInline:!0}),i0.ɵɵngDeclareClassMetadata({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicViewComponent,decorators:[{type:Component,args:[{selector:"TwicView",template:"<ng-content></ng-content>"}]}],ctorParameters:function(){return[{type:i0.Renderer2},{type:i0.ElementRef}]}});class TwicPicsComponentsModule{}TwicPicsComponentsModule.ɵfac=i0.ɵɵngDeclareFactory({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicPicsComponentsModule,deps:[],target:i0.ɵɵFactoryTarget.NgModule}),TwicPicsComponentsModule.ɵmod=i0.ɵɵngDeclareNgModule({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicPicsComponentsModule,declarations:[TwicBackgroundComponent,TwicImgComponent,TwicMediaComponent,TwicPictureComponent,TwicVideoComponent,TwicViewComponent],imports:[CommonModule],exports:[TwicBackgroundComponent,TwicImgComponent,TwicPictureComponent,TwicVideoComponent,TwicViewComponent]}),TwicPicsComponentsModule.ɵinj=i0.ɵɵngDeclareInjector({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicPicsComponentsModule,imports:[[CommonModule]]}),i0.ɵɵngDeclareClassMetadata({minVersion:"12.0.0",version:"12.2.0",ngImport:i0,type:TwicPicsComponentsModule,decorators:[{type:NgModule,args:[{declarations:[TwicBackgroundComponent,TwicImgComponent,TwicMediaComponent,TwicPictureComponent,TwicVideoComponent,TwicViewComponent],imports:[CommonModule],exports:[TwicBackgroundComponent,TwicImgComponent,TwicPictureComponent,TwicVideoComponent,TwicViewComponent]}]}]});const VERSION="v1",parametersMap=[["anticipation","anticipation"],["class","class"],["maxDPR","max-dpr"],["step","step"]],register=options=>{if(isBrowser&&!isReactNative){const parts=[config.domain+"/?"+VERSION];parametersMap.forEach(p=>{var[p,actualKey]=p;options.hasOwnProperty(p)&&options[p]&&parts.push(actualKey+"="+options[p])});var scriptElementId=options["scriptElementId"];createElement({element:document.head,value:[{attributes:{rel:"preconnect",href:config.domain},elementName:"link"},{attributes:Object.assign(Object.assign({async:"",defer:""},scriptElementId&&{id:scriptElementId}),{src:parts.join("&")}),elementName:"script"}]});const style=createElement({elementName:"style",value:configBasedStyle()});document.head.appendChild(style),document.addEventListener("astro:after-swap",()=>document.head.appendChild(style))}},validate=options=>{options||throwError("install options not provided");var{domain:options,env,path}=options;options&&rValidDomain.test(options)||throwError(`install domain "${options}" is invalid`),path&&rInvalidPath.test(path)&&throwError(`install path "${path}" is invalid`),env&&!rValidEnvironment.test(env)&&throwError(`install env "${env}" is invalid`)},installTwicPics=options=>{validate(options);var{domain,env,path}=options;setConfig(Object.assign(Object.assign({},options),{domain:parseDomain(domain),env:parseEnv(env),path:parsePath(path)})),register(options)},installTwicpics=installTwicPics;export{TwicBackgroundComponent,TwicImgComponent,TwicMediaComponent,TwicPicsComponentsModule,TwicPictureComponent,TwicVideoComponent,TwicViewComponent,installTwicPics,installTwicpics};
